...
var $author$project$Advent8$findVisibleTrees = F3(
	function (currentHighest, processedTrees, trees) {
		findVisibleTrees:
		while (true) {
			if (!trees.b) {
				return $elm$core$List$reverse(processedTrees);
			} else {
				var head = trees.a;
				var rest = trees.b;
				if (_Utils_cmp(head.a, currentHighest) > 0) {
					var $temp$currentHighest = head.a,
						$temp$processedTrees = A2(
						$elm$core$List$cons,
						_Utils_Tuple2(head.a, true),
						processedTrees),
						$temp$trees = rest;
					currentHighest = $temp$currentHighest;
					processedTrees = $temp$processedTrees;
					trees = $temp$trees;
					continue findVisibleTrees;
				} else {
					var $temp$currentHighest = currentHighest,
						$temp$processedTrees = A2($elm$core$List$cons, head, processedTrees),
						$temp$trees = rest;
					currentHighest = $temp$currentHighest;
					processedTrees = $temp$processedTrees;
					trees = $temp$trees;
		...
		A2(
			...
			A2(
				...
				A2(
					$elm$core$Maybe$map,
					$elm$core$List$map(
						A2($author$project$Advent8$findVisibleTrees, -1, _List_Nil)),
					A2(
						$elm$core$Maybe$map,
